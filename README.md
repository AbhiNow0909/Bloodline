...
The PDF test fixture is synthetic and generated by `tests/synthetic_pdf.py`; after changing
the generator, regenerate it with `uv run python -m tests.synthetic_pdf` (a test checks they
match). See [`backend/tests/fixtures/README.md`](backend/tests/fixtures/README.md).
...
- Identity fields are parsed and removed locally. From each results page only the results
  table is kept (the identity header and the signature footer are dropped), then names,
  barcodes, doctors, emails and phone numbers are scrubbed, and extraction **fails closed** if
  any identity data remains. The LLM sees only that text, and the query agent refers to
  "the patient" (or "Member A", "Member B" in family chat), never a name.
